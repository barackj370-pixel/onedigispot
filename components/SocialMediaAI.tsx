...
const getPlatformIcon = (platform: string) => {
  switch(platform) {
    ...
    case 'TikTok': return <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M9 12a4 4 0 1 0 4 4V4a5 5 0 0 0 5 5"/></svg>;
...
const getPlatformColor = (platform: string) => {
  switch(platform) {
    case 'LinkedIn': return 'bg-[#0A66C2]/10 border-[#0A66C2] text-[#0A66C2]';
    case 'Twitter': return 'bg-black/5 border-black text-black';
    case 'Instagram': return 'bg-[#E1306C]/10 border-[#E1306C] text-[#E1306C]';
    case 'Facebook': return 'bg-[#1877F2]/10 border-[#1877F2] text-[#1877F2]';
    case 'TikTok': return 'bg-black/5 border-black text-black';
    default: return 'bg-indigo-50 border-indigo-600 text-indigo-600';
  }
};
  ...
  useEffect(() => {
    const handleMessage = (event: MessageEvent) => {
      ...
      if (event.data?.type === 'OAUTH_AUTH_SUCCESS') {
        ...
        // Also automatically select it for posting
        setPlatforms(prev => 
          prev.includes(platform) ? prev : [...prev, platform]
        );
        if (event.data.token) {
    ...
    <section id="social-media-ai" className="py-12 bg-slate-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center max-w-3xl mx-auto mb-16">
          ...
          <h2 className="text-4xl md:text-5xl font-extrabold text-slate-900 mb-6">Social Media AI Post Master</h2>
          ...
          <div className="lg:col-span-1 space-y-8">
            <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm">
              ...
              <div className="space-y-6">
                {/* Connected Accounts */}
                <div>
                  <label className="block text-sm font-bold text-slate-700 mb-3">1. Connect Accounts</label>
                  <div className="flex flex-wrap gap-2 mb-4">
                    {['TikTok', 'LinkedIn', 'Twitter', 'Instagram', 'Facebook'].map(platform => {
                ...
                <div>
                  <label className="block text-sm font-bold text-slate-700 mb-3">2. Target Platforms for Post</label>
                  <div className="grid grid-cols-2 gap-3 mb-4">
                    {connectedPlatforms.length === 0 ? (
                      <div className="col-span-2 text-sm text-slate-500 italic p-4 bg-slate-50 rounded-xl border border-slate-200 text-center">
                        Please connect an account above to select it as a target.
                      </div>
                    ) : (
                      ['TikTok', 'LinkedIn', 'Twitter', 'Instagram', 'Facebook'].map(platform => {
                        if (!connectedPlatforms.includes(platform)) return null;
                        return (
                          <button 
                            key={platform}
                            onClick={() => togglePlatform(platform)}
                            className={`flex items-center gap-2 p-3 rounded-xl border font-medium transition-all ${platforms.includes(platform) ? getPlatformColor(platform) : 'bg-white border-slate-200 text-slate-500 hover:border-slate-300'}`}
                          >
                  ...
                  <div className="bg-blue-50 border border-blue-200 text-blue-800 p-4 rounded-xl text-xs leading-relaxed">
                    <strong>How Auto-Posting Works:</strong> We have successfully integrated TikTok's real API! By connecting your TikTok account in Step 1, creating a video, and clicking "Share" below, you will send actual video bytes straight to the authentic TikTok processing queue. <em>Other platforms are currently connecting in "mock" mode and will be fully integrated soon.</em>
                  </div>
                </div>

                <div>
                  <label className="block text-sm font-bold text-slate-700 mb-2">3. Seed Topic</label>
                  <textarea 
                    value={topic}
